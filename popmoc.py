...
def sigma0_grid_86L():
    """ 
    Defines an 86-layer sigma0 grid for MOC(sigma0) computation, returning mid-points and edge-points
    as xarrays (for use with xhistogram).
    """
    tmp1 = np.arange(23.4,26,0.2)
    tmp2 = np.arange(26,27,0.1)
    tmp3 = np.arange(27,28.5,0.05)
    sig0 = np.concatenate((tmp1,tmp2,tmp3))
    nsig = len(sig0)
    # Define density of midpoint, top, and bottom of isopycnal layers
    sigma_mid=xr.DataArray(sig0,coords={'sigma':sig0},
                           attrs={'long_name':'Sigma0 at middle of layer','units':'kg/m^3'})
    sigma_edge = (sigma_mid+sigma_mid.shift(sigma=1))/2.
    sigma_edge[0] = 0.
    sigma_edge = np.append(sigma_edge.values,[50.])
    sigma_edge=xr.DataArray(sigma_edge,coords={'sigma':sigma_edge},
                           attrs={'long_name':'Sigma0 at edges of layer','units':'kg/m^3'})
    return sigma_mid,sigma_edge
...
def wflux_zonal_sum(wflux,regionmask,lat):
    ...
    """
    wgts = wflux*regionmask[:,:,np.newaxis]
...
def compute_MOC_sector(wflux,regionmask,lat):
    """ 
    Use w-method to compute MOC.
